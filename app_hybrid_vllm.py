...
CPU_THREAD_WORKERS = 10
PAGE_CONCURRENCY_LIMIT = 2
JOB_THREAD_WORKERS = 8
OCR_POOL_SIZE = 8
PDF_DPI = 120
MAX_IMAGE_SIZE = 1200
MAX_TOKENS = 300
...
ocr_pool = queue.Queue()


def create_paddle_ocr_instance(instance_id: int):
    print(f"[OCR] Initializing PaddleOCR instance {instance_id}...")

    ocr = PaddleOCR(
        det_model_dir="/home/rohit.sahu/Qwen_model/models/ch_PP-OCRv4_det_infer",
        use_angle_cls=False,
        lang="en",
        use_gpu=False,
        rec_batch_num=4,
    ...
    print(f"[OCR] PaddleOCR instance {instance_id} initialized")
    return ocr


def initialize_ocr_pool():
    if not ocr_pool.empty():
        return

    print(f"[OCR] Creating OCR object pool. size={OCR_POOL_SIZE}")

    for i in range(OCR_POOL_SIZE):
        ocr_pool.put(create_paddle_ocr_instance(i + 1))

    print("[OCR] OCR object pool ready")


def run_paddle_ocr_blocking(images: List[Image.Image]) -> Dict[str, List[Dict[str, Any]]]:
    ocr_result: Dict[str, List[Dict[str, Any]]] = {}

    print("[OCR] Waiting for OCR object from pool...")
    ocr = ocr_pool.get()
    print("[OCR] OCR object borrowed from pool")
    ...
    try:
        ...
        print("[OCR] Returning OCR object back to pool")
        ocr_pool.put(ocr)
...
async def process_job(job_id: str):
    ...
    try:
        ...
        log_gpu_metrics_csv(
            ...
            status="processing",
            stage="before_parallel_ocr_vllm",
        ...
        ocr_task = asyncio.create_task(run_paddle_ocr_async(images))
        vllm_task = asyncio.create_task(run_vllm_inference_on_images(images))

        paddle_ocr_output, vllm_result = await asyncio.gather(
            ocr_task,
            vllm_task,
        )
        ...
        print(f"[JOB] {job_id} PaddleOCR and vLLM completed")
...
async def startup_event():
    ...
    print(f"[INFO] OCR pool size: {OCR_POOL_SIZE}")
